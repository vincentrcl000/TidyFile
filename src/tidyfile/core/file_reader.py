...
class FileReader:
    ...
    def extract_path_tags(self, file_path: str, base_folder: str = None) -> Dict[str, str]:
        ...
        try:
            from tidyfile.utils.path_utils import extract_path_tags as extract_tags
    ...
    def _find_available_multimodal_model(self) -> str:
        ...
        # 使用统一的AI管理器获取可用模型
        try:
            from tidyfile.ai.client_manager import get_available_models
...
摘要：/no_think"""
    ...
    def append_result_to_file(self, ai_result_file: str, result: dict, base_folder: str = "") -> None:
        ...
        try:
            ...
            # 使用并发管理器写入结果
            try:
                from tidyfile.core.concurrent_result_manager import append_file_reader_result
