...
/no_think"""
    ...
    def _build_tags_with_chain(self, result: dict) -> Dict[str, str]:
        ...
        try:
            from tidyfile.utils.path_utils import build_tags_with_chain
    ...
    def append_result_to_file(self, ai_result_file: str, result: dict, target_directory: str = "") -> None:
        ...
        try:
            ...
            if final_target_path:
                try:
                    from tidyfile.utils.path_utils import extract_chain_tags_from_final_path, build_chain_tags
            ...
            # 使用并发管理器写入结果
            try:
                from tidyfile.core.concurrent_result_manager import append_classification_result
